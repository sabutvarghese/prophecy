...
def pipeline(spark: SparkSession) -> None:
    ...
    df_AggByAccount = AggByAccount(spark, df_select_fields)
    df_salesforce_Account = salesforce_Account(spark)
    df_by_id_left_outer = by_id_left_outer(spark, df_salesforce_Account, df_AggByAccount)
    enriched_accounts(spark, df_by_id_left_outer)
